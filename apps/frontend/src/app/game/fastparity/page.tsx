...
export default function FastParityPage() {
  ...
  const [gameParams, setGameParams] = useState<FastParityGameParams>({ betType: 'color', value: 'red' });
  ...
  const placeBet = async () => {
    ...
    setLoading(true);
    try {
      const response = await betAPI.place({ gameType: 'FASTPARITY', currency: 'USD', amount, gameParams });
  ...
  const handleStartAutoBet = async (config: AutoBetConfig) => {
    try {
      await betAPI.startAutobet({ gameType: 'FASTPARITY', currency: 'USD', amount, gameParams, config });
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2">
            <div className="card">
              <h2 className="text-2xl font-bold mb-6">Fast Parity</h2>
              ...
              {result && (
                <div className={`mb-6 p-6 rounded-lg text-center ${result.won ? 'bg-green-900/20 border border-green-500' : 'bg-red-900/20 border border-red-500'}`}>
                  <div className="text-6xl font-bold mb-2 w-20 h-20 mx-auto rounded-full flex items-center justify-center text-white" style={getColorStyle(result.color)}>
                    {result.number}
                  </div>
                  <div className="text-2xl mb-2 capitalize">{result.color} - {result.isEven ? 'Even' : 'Odd'}</div>
                  <div className="text-xl">{result.won ? '🎉 WIN!' : '😢 LOST'}</div>
                </div>
              )}

              <FastParityGameControls onChange={setGameParams} disabled={loading || autoBetActive} />
            </div>
          </div>
